  ...
  while (pair != NULL && pair->code != 0)
    {
      switch (pair->code)
        ...
        case 279:
          ...
          if (j < 0 || j >= 6)
            {
              LOG_ERROR ("Illegal code %d", pair->code);
              return NULL;
           }
          border = &borders[j];
          assert (border);
          border->linewt = dxf_find_lweight (pair->value.i);
          LOG_TRACE ("%s.rowstyles[%d].borders[%d].linewt = %d [BSd %d]\n",
                     obj->name, i, j, border->linewt, pair->code);
          break;
        case 284:
        case 285:
        case 286:
        ...
        case 69:
          ...
          border = &borders[j];
          assert (border);
          border->color.index = pair->value.i;
